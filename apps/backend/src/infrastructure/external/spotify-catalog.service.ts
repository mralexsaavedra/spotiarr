import { AlbumType, ArtistRelease, NormalizedTrack, SpotifySearchResults } from "@spotiarr/shared";
...
export class SpotifyCatalogService extends SpotifyHttpClient {
  ...
  async searchCatalog(
    ...
    try {
      ...
      // Use separate limits per type — artists intentionally capped low to avoid
      // returning unrelated artists that only have tangential connections.
      const trackLimit = limits.track ?? 10;
      const albumLimit = limits.album ?? 10;
      const artistLimit = limits.artist ?? 5;

      // Build a single query per unique limit group to minimise API calls.
      // Group types that share the same limit together.
          ...
          if (data.tracks?.items) {
            ...
            result.tracks = await Promise.all(
              data.tracks.items.map(async (track: SpotifyTrack, index: number) => {
                ...
                const primaryArtistImage = await getPrimaryArtistImage(primaryArtistId);
                const normalized = this.mapToNormalizedTrack(track, { primaryArtistImage });
