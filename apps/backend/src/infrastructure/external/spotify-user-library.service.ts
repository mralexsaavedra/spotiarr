...
export class SpotifyUserLibraryService extends SpotifyHttpClient {
  ...
  async getActiveArtistReleases(
      ...
      if (!albumsResponse.ok) {
        ...
        if (albumsResponse.status === 429) {
          throw new AppError(
            429,
            "spotify_rate_limited",
            "Rate limited by Spotify API while fetching active artist releases.",
          );
        }

        this.log(
          `Failed to fetch releases for artist ${artist.id}: ${albumsResponse.status} ${errorText}`,
          "warn",
        );
        releasesPerArtist.push([]);
        continue;
    ...
    const recent = releasesPerArtist.flat().filter((item) => {
      if (!item.releaseDate) return false;
