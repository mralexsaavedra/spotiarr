...
const PlaylistTrackItem = memo(({ track, index, onRetryTrack }: PlaylistTrackItemProps) => {
  ...
  return (
    <div className="space-y-1">
      {tracks.map((track, index) => (
        <PlaylistTrackItem key={track.id} track={track} index={index} onRetryTrack={onRetryTrack} />
      ))}
    </div>
  );
};
