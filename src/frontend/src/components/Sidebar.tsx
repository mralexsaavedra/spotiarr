...
export const Sidebar = ({ pathname, version, toggleDarkMode, isDarkMode }: { pathname: string, version: string; toggleDarkMode: () => void; isDarkMode: boolean }) => (
      ...
      <nav className="flex flex-col gap-4">
        {NAV_ITEMS.map(item => {
          ...
          return (
            ...
            >
              <i className={`fa-solid ${item.icon} group-hover:text-spotify-green group-hover:dark:text-spotify-green transition ${active ? 'text-spotify-green' : ''}`} />
              <span className={`group-hover:text-spotify-green group-hover:dark:text-spotify-green transition ${active ? 'text-spotify-green' : ''}`}>{item.label}</span>
            </Link>
          );
        })}
      </nav>
