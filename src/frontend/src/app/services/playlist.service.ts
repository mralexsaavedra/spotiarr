...
export class PlaylistService {
  ...
  all$ = this.store
    ...
    .pipe(
      unionEntities(),
      map((data) =>
        data.sort((a, b) => this.groupActiveAndSortByCreation(a, b)),
      ),
    );
  createLoading$ = this.store.pipe(
    joinRequestResult([CREATE_LOADING], { initialStatus: 'idle' }),
  );
  ...
  getCompletedTrackCount(id: number): Observable<number> {
    return this.trackService
      .getCompletedByPlaylist(id)
      .pipe(map((data) => data.length));
  }

  getErrorTrackCount(id: number): Observable<number> {
    return this.trackService
      .getErrorByPlaylist(id)
      .pipe(map((data) => data.length));
  }
  ...
  getStatus$(id: number): Observable<PlaylistStatusEnum> {
    return combineLatest([
      ...
      map(([playlist, trackCount, completedCount, errorCount]) => {
    ...
    this.all$
      .pipe(
        ...
        switchMap((playlists) =>
          combineLatest(
            playlists.map((item) =>
              this.deleteIfStatusEquals$(item.id, status),
            ),
          ),
        ),
      )
  ...
  private deleteIfStatusEquals$(
    id: number,
    status2Filter: PlaylistStatusEnum,
  ): Observable<void> {
      ...
      .pipe(
        tap((data: Playlist[]) =>
          this.store.update(
            setEntities(data),
            setEntities(
              data.map((item) => ({ id: item.id, collapsed: false })),
              { ref: UIEntitiesRef },
            ),
          ),
        ),
        tap((data: Playlist[]) =>
          data.forEach((playlist) => this.trackService.fetch(playlist.id)),
        ),
      )
      .subscribe();
  }
  ...
  create(spotifyUrl: string): void {
    this.http
      .post(ENDPOINT, { spotifyUrl })
      .pipe(trackRequestResult([CREATE_LOADING], { skipCache: true }))
      .subscribe();
  }

  toggleCollapsed(id: number): void {
    this.store.update(
      updateEntities(id, (old) => ({ ...old, collapsed: !old.collapsed }), {
        ref: UIEntitiesRef,
      }),
    );
  }
  ...
  private groupActiveAndSortByCreation(
    a: Playlist & PlaylistUi,
    b: Playlist & PlaylistUi,
  ): number {
  ...
  private initWsConnection(): void {
    ...
    this.socket.on(WsPlaylistOperation.New, (playlist: Playlist) =>
      this.store.update(
        ...
        upsertEntities(
          { id: playlist.id, collapsed: false },
          { ref: UIEntitiesRef },
        ),
      ),
    );
